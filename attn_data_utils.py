...
class DataLoader:
    ...
    def __init__(self, data_path: str, data_type: str, n_shape_coeff:int, fixed_geom: bool):
        ...
        n_shape_coeff: int >= 0
            The number of shape coefficients to input the GNN emulator
        fixed_geom: bool
        ...
        sparse_topology =  np.load(f'{topology_save_dir}/sparse-topology.npy').astype(jnp.int32)
        self._senders = sparse_topology[:,0]
        self._receivers = sparse_topology[:,-1]

        # node features
        self._nodes = np.load(f'{data_save_dir}/attn-node-features.npy').astype('float32')
    ...
    def get_graph(self, data_idx):
        ...
        if self._fixed_geom:
            ...
            return device_put(self._nodes[data_idx]), device_put(self._theta_vals[data_idx:(data_idx+1)]), device_put(self._shape_coeffs[data_idx]), device_put(self._displacement[data_idx])
    ...
    def return_index_0(self):
        ...
        return device_put(self._nodes[0]), device_put(self._theta_vals[0:1]), device_put(self._shape_coeffs[0]), device_put(self._displacement[0])
