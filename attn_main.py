...
import attn_data_utils as data_utils
import attn_utils as utils
import functools
...
flags.DEFINE_enum('mode', 'train', ['train', 'evaluate', 'evaluate_all'],
                  help='Fit model to training data or evaluate on test data')
flags.DEFINE_integer('n_epochs', 500, lower_bound=1, help='Number of epochs to train the model for')
...
flags.DEFINE_float('lr', 2e-4, lower_bound=0, help='Learning rate for training the network')
...
def predict_norm_displacement(V, theta, z_global, params, net):
    return net.apply(params, V, theta, z_global)


def predict_displacement(V, theta, z_global, params, Umean, Ustd, net):
    return predict_norm_displacement(V, theta, z_global, params, net) * Ustd + Umean


def compute_train_loss(params, net, input_output_tuple):
    """Compute (mean) MSE between true and predicted displacement (normalised)"""
    V, theta, z_global, U = input_output_tuple
    Upred = predict_norm_displacement(V, theta, z_global, params, net)
...
def compute_test_loss(params, input_output_tuple, Umean, Ustd, net):
    ...
    V, theta, z_global, U = input_output_tuple
    Upred = predict_displacement(V, theta, z_global, params, Umean, Ustd, net)
    return mean_rmse(U, Upred)
...
def train_step(state, input_output_tuple, net, train_loss_fn):
    ...
    grad_fn = jax.value_and_grad(partial_loss_fn)  # return a function
    loss, grads = grad_fn(state.target)  # Calculate the gradient and loss value of state.target
    state = state.apply_gradient(grads=grads)  # Optimize parameters using gradient descent
    return state, loss
...
def emul_pred_fn_vg(input_output_tuple, params, Umean, Ustd, net):
    ...
    V, theta, z_global, _ = input_output_tuple
    return predict_displacement(V, theta, z_global, params, Umean, Ustd, net)
...
def train(data_path, n_shape_coeff, n_epochs, lr, trained_params_dir, fixed_geom, dir_label):
    ...
    results_save_dir = create_savedir(data_path, n_shape_coeff, n_epochs, lr, dir_label)
    ...
    config_dict = create_config_dict(n_shape_coeff, n_epochs, lr, train_data._output_dim)
    ...
    model, params = utils.initialise_emulator(config_dict, train_data, trained_params_dir)
    ...
    for epoch_idx in range(n_epochs):
        ...
        min_valid_loss = utils.save_trained_params(valid_loss, min_valid_loss, state.target, epoch_idx, n_epochs,
                                                   results_save_dir)
...
def evaluate(data_path, K, n_shape_coeff, n_epochs, lr, trained_params_dir, fixed_geom, dir_label):
    ...
    model, trained_params = utils.initialise_emulator(config_dict, test_data, results_save_dir, fixed_geom,
                                                      trained_params_dir)
    ...
    Uorig = test_data._displacement

    logging.info('Saving Results')
    logging.info(f'Predicted displacement: {results_save_dir}/predDisplacement.npy')
    logging.info(f'True displacement: {results_save_dir}/trueDisplacement.npy')
    np.save(f'{results_save_dir}/predDisplacement.npy', Upred)
    np.save(f'{results_save_dir}/trueDisplacement.npy', Uorig)
...
def evaluate_all(data_path, K, n_shape_coeff, n_epochs, lr, trained_params_dir, fixed_geom, dir_label):
    ...
    # load validation data
    validation_data = data_utils.DataLoader(data_path, 'validation', n_shape_coeff, fixed_geom)
    logging.info(f'Number of validation data points: {validation_data._data_size}')
    ...
    logging.info('Predicting on test data set using trained emulator')
    Upred_test = predict_dataset(test_data, prediction_fn_jit)
    ...
    Upred_train = predict_dataset(train_data, prediction_fn_jit)

    # Validation data
    # initialise GNN emulator and read trained network parameters
    model, trained_params = utils.initialise_emulator(config_dict, validation_data, results_save_dir, fixed_geom,
                                                      trained_params_dir)
    ...
    prediction_fn_jit = jit(functools.partial(emul_pred_fn,
                                              net=model,
                                              params=trained_params,
                                              Umean=validation_data._displacement_mean,
                                              Ustd=validation_data._displacement_std))

    Upred_validation = predict_dataset(validation_data, prediction_fn_jit)

    # Original data
    # Uorig = np.concatenate((train_data._displacement, validation_data._displacement, test_data._displacement), axis = 0)
    Uorig_train = train_data._displacement
    Uorig_test = test_data._displacement
    Uorig_validation = validation_data._displacement

    logging.info('Predicting on train data set using trained emulator')
    utils.print_error_statistics(Uorig_train, Upred_train, logging)
    logging.info('Predicting on validation data set using trained emulator')
    utils.print_error_statistics(Uorig_test, Upred_test, logging)
    logging.info('Predicting on test data set using trained emulator')
    utils.print_error_statistics(Uorig_validation, Upred_validation, logging)

    Upred = np.concatenate((Upred_train, Upred_validation, Upred_test), axis=0)
    Uorig = np.concatenate((train_data._displacement, validation_data._displacement, test_data._displacement), axis=0)
    # real_node_coords_all = train_data._real_node_coords

    real_node_coords_all = np.concatenate(
        (train_data._real_node_coords, validation_data._real_node_coords, test_data._real_node_coords), axis=0)

    logging.info('Saving Results')
    logging.info(f'Predicted displacement: {results_save_dir}/predDisplacement.npy')
    logging.info(f'True displacement: {results_save_dir}/trueDisplacement.npy')
    np.save(f'{results_save_dir}/predDisplacement_all.npy', Upred)
    np.save(f'{results_save_dir}/trueDisplacement_all.npy', Uorig)

    # save coordinates of test geometries in reference configuration
    np.save(f'{results_save_dir}/referenceCoords_all.npy', real_node_coords_all)

    # print prediction error statistics to console
    utils.print_error_statistics(Uorig, Upred, logging)
