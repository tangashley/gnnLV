...
import attn_data_process_utils as utils
...
COPY_FILENAMES: Sequence[str] = ['real-node-coords']

# data which needs to be normalised before saving to final processed data dir
NORMALISE_FILENAMES: Sequence[str] = ['attn-node-features', 'global-features', 'shape-coeffs', 'real-node-displacement']
...
def main(_):
    ...
    if FLAGS.mode in ['run_all', 'generate_topology']:
        utils.attn_node_feature(FLAGS.data_dir,
                                          DATA_TYPES)
